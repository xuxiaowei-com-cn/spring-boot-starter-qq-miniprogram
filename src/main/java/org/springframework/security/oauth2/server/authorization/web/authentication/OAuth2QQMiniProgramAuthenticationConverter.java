...
import org.springframework.security.oauth2.server.authorization.authentication.OAuth2QQMiniProgramAuthenticationToken;
...
/**
 * QQ OAuth2 用于验证授权授予的 {@link OAuth2QQMiniProgramAuthenticationToken}。
 *
 * @author xuxiaowei
 * @since Joe Grandja
 * @since 0.0.1
 ...
 */
public class OAuth2QQMiniProgramAuthenticationConverter implements AuthenticationConverter {
	...
	public Authentication convert(HttpServletRequest request) {
		...
		if (!OAuth2QQMiniProgramAuthenticationToken.QQ_MINIPROGRAM.getValue().equals(grantType)) {
			return null;
		}
		...
		// appid (REQUIRED)
		String appid = parameters.getFirst(OAuth2QQMiniProgramParameterNames.APPID);
		if (!StringUtils.hasText(appid)) {
			...
			while (headers.hasMoreElements()) {
				...
				if (StringUtils.hasText(nextElement)) {
					...
					if (split.length > 3) {
						appid = split[3];
						parameters.add(OAuth2QQMiniProgramParameterNames.APPID, appid);
					}
				}
			}
		}
		if (!StringUtils.hasText(appid) || parameters.get(OAuth2QQMiniProgramParameterNames.APPID).size() != 1) {
			OAuth2EndpointUtils.throwError(OAuth2ErrorCodes.INVALID_REQUEST, OAuth2QQMiniProgramParameterNames.APPID,
					OAuth2QQMiniProgramEndpointUtils.AUTH_CODE2SESSION_URI);
		}
		...
		parameters.forEach((key, value) -> {
			if (!key.equals(OAuth2ParameterNames.GRANT_TYPE) && !key.equals(OAuth2ParameterNames.CLIENT_ID)
					...
					&& !key.equals(OAuth2QQMiniProgramParameterNames.APPID)
					&& !key.equals(OAuth2ParameterNames.SCOPE)) {
				additionalParameters.put(key, value.get(0));
			}
		});

		return new OAuth2QQMiniProgramAuthenticationToken(clientPrincipal, additionalParameters, appid, code, scope);
	}
